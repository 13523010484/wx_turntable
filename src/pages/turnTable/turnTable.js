...
Page({
    ...
    // 页面初始化加载时
    onLoad() {
        var aniData = wx.createAnimation({ //创建动画对象
            duration: 2000,
            timingFunction: 'ease'
        });
    ...
    setPlateData() {
        ...
        if (lotteryArrLen < 2) { //数组的奖品只有一个，扩展数组的长度到4
            var evenArr = new Array(4); //创建一个数组，方便操作。
            ...
            var dataLen = 0; //用来放原来数组的索引
            var evenArr = new Array(lotteryArrLen * 2); //创建扩展数组
            for (var i = 0; i < (lotteryArrLen * 2); i++) {
                if (i % 2 == 1) {
    ...
    startRollTap() {
        ...
        if (canRoll) {
            canRoll = false;
            var aniData = this.aniData; //获取this对象上的动画对象
